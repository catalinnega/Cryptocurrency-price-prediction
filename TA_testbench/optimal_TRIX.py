...
feature_dicts = feature_list_test5.get_features_dicts()
...
for tau in tau_values:
    ...
    test_train_data, h ,l = xgb_obj.get_test_and_train_data(label_window = 1, 
...
import numpy as np
a = test_train_data['X_train']
a = np.reshape(a, (np.shape(a)[0], 1, np.shape(a)[1]))
b = test_train_data['Y_train']
#b = np.reshape(b, (np.shape(b)[0], 1, np.shape(b)[1]))

model = tf.keras.models.Sequential()
model.add(tf.keras.layers.LSTM(units= 100,activation= 'tanh',input_shape=(np.shape(a)[1], np.shape(a)[2])))
model.add(tf.keras.layers.Dropout(rate= 0.2))
model.add(tf.keras.layers.Dense(units= 1,  activation= 'linear'))
model.compile(optimizer= 'adam', loss= 'mae')


model.fit(a, test_train_data['Y_train'], 
...
#mae_key =  [str(i) for i in list(results.keys())]
...
#import pickle
#with open('atr_100.pkl', 'wb') as f:
#    pickle.dump(results, f)

#with open('atr_100.pkl', 'rb') as f:
...
plt.figure()
plt.title("Performanta indicatorului TRIX")
plt.plot(mae_key, mae_val, label = 'Eroarea medie absoluta')
#plt.xticks(rotation=90)
plt.xlabel('Lungimea ferestrei in esantioane')
plt.ylabel('Valoarea MAE')
#plt.axvline(keys_auc_train[a.index(max(a))], label = 'max value', color = 'r')
plt.annotate('Minim. TRIX = ' +  str(key), xy=(key, val), xytext=(key*4.5, val*1.002),
