...
#    
    ...
    def estimator_fit(self, regression = True, num_classes = 2, num_trees = None, max_nodes = 1000, max_fertile_nodes = 0, rootLogger = None):
        #### Random Forest 
...
for analysis_set in feature_sets[20:]:
    ...
    for secondary_set in feature_sets:
        if(analysis_set != secondary_set):
            ...
            print('####              ', features)
            rf = get_data_RF()
            dataset_path, log_path, rootLogger = init_paths()
            data = rf.get_input_data(database_path = dataset_path, 
            ...
            test_train_eth = rf.get_test_and_train_data()
            rf.estimator_fit(rootLogger = rootLogger) ##  creates a new estimator
            ft,ft_c = rf.get_feature_importances(log_path = log_path)
            if(not ft):
...
#eth = get_data_RF()
#data_eth = eth.get_input_data(database_path = '/home/catalin/git_workspace/disertatie/databases/klines_2014-2018_15min/', 
#                              feature_names = feature_names, 
#                              preprocessing_constant = 0.99, 
#                              normalization_method = 'rescale',
#                              skip_preprocessing = False,
...
##with open('/home/catalin/git_workspace/disertatie/test.csv', 'w') as f:
##    for key in ft.keys():
##        f.write("%s,%s\n"%(key,ft[key]))
##
##f.close()
